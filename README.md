...
| **claude** | `bili claude` (launcher) or `bili plugin install claude` (native posture, #964 — managed settings block + session-owned proxy; see the notes below). On AWS Bedrock (`CLAUDE_CODE_USE_BEDROCK`), `bili claude` compresses the Bedrock wire too — see [CONFIGURATION.md › Claude on AWS Bedrock](CONFIGURATION.md#claude-on-aws-bedrock) |
