...
- **Profile 安装下的已知局限 —— `llm-pi-ai` 传输(#1158):** 由 dsh 的 `llm-pi-ai` 层服务的 provider(自定义 `llm-pi-ai.providers.*` 路由)把自己持有的 fetch 实现直接传给 OpenAI SDK,模型请求因此**从不经过**本泳道依赖的全局 fetch 补丁 —— 它们完全绕过代理,压缩对它们静默失效(用普通全局 fetch 的第三方 provider 不受影响)。检测:此类会话调用 bili 工具而从未有模型请求到达代理时,代理会打一次性 `[plugin] NO MODEL REQUESTS seen for conversation …` 告警,工具报错里也带同样指引。解法:改用 `bili dsh` 启动 —— 启动器的 settings overlay 会把那些 provider 的 `baseURL` 重写为 `/bili/` URL,无论传输层注入哪种 fetch,流量都必然过代理。
