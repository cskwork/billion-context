...
- **Type:** `object` (`{ enabled?, minTokens?, maxStoreBytes? }`)
- **Default:** *(disabled — the feature is off unless you set `enabled: true`)*
- **Status:** ACTIVE (v1 — proxy mode, native-tools wires only)
- **Description:** Opt-in **content-addressed message store** / built-in CCR (issue #1097). Instead of force-distilling oversized tool results (like `absorb`) or carrying them on the wire forever, the original goes into a per-session content store at arrival and the wire keeps a deterministic, byte-stable placeholder (`📦 [stored #m00423 · shell · 4213 tok] "head preview" → acp_retrieve("m00423")`). The model retrieves the full original on demand via the injected `acp_retrieve` tool; a retrieve is ephemeral (it rides the intra-request tool-result channel, never enters fold space, consumes no message ref). Lossless by default: a retrieve not made costs one cheap tool call; a detail distilled away by absorb is gone for good. Sub-fields (merged deepest-wins like every other CompressSettings field):
  - `enabled: boolean` — master switch; anything other than `true` keeps the feature fully off (no placeholders, no tool).
  - `minTokens: number` — only tool results at or above this many tokens are ID-referenced (default `500`); smaller results stay verbatim.
  - `maxStoreBytes: number` — per-session cap on stored bytes, summed logically per ref (default `2 MiB`); content beyond the cap stays verbatim rather than being emitted as an unretrievable placeholder.
  The index is metadata-only and persists inside the session JSON alongside `blockContents`; payloads live in hash-keyed sidecar files (`<sha256(sessionId)>/<sha1(text)>.txt`) under `storeDir()` (env `BILI_STORE_DIR`, default `<stateDir>/store`), deduped by content hash and lazy-read on retrieve. Sidecars use the same at-rest codec as the session file when `BILI_ENCRYPTION_KEY` is set. Only the *content* inside a `tool` result shrinks — pairing with the assistant `tool_calls` is untouched. v1 scope gates: **proxy mode only** (plugin agents would need `acp_retrieve` advertised in their plugin manifest before a placeholder could be emitted — otherwise silent loss), and **native-tools wires only**: responses marker/text-protocol routes and `ACP_NO_INJECT_TOOL` have no channel to execute the retrieve, so the store disarms itself there instead of losing content. Per-session stats (stored bytes, current wire bytes saved, retrieve rate) surface in `acp_status`; each retrieve logs a `[store] retrieve …` line.
...
| `BILI_MAX_SESSIONS` | Max sessions held in memory (default `256`; LRU eviction — disk is the source of truth). |
 ...
 | `BILI_STORE_DIR` | Directory for content-store sidecar payloads (`compress.store`, #1097): hash-keyed files under `<sha256(sessionId)>/<sha1(text)>.txt`, lazy-read on `acp_retrieve`. Default `<stateDir>/store`. Sidecars use the session file's at-rest codec when `BILI_ENCRYPTION_KEY` is set. |
...
| `bili aider [opts --] [args]` | Proxy + **Aider** (Python pair programmer) — cert-MITM via `HTTPS_PROXY` + `SSL_CERT_FILE`/`REQUESTS_CA_BUNDLE`; endpoint discovered from inherited env (`OPENAI_API_BASE`, `OPENAI_BASE_URL`, `ANTHROPIC_API_BASE`, `ANTHROPIC_BASE_URL`, `GEMINI_API_BASE`, `DEEPSEEK_API_BASE`) or `.aider.conf.yml` (`openai-api-base:`), with `--openai-api-base` / `--set-env` args passed after the client taking precedence; nothing declared → `api.openai.com` + `api.anthropic.com` whitelisted by default; non-loopback plain-http endpoints ride `HTTP_PROXY` absolute-form, loopback stays direct via `NO_PROXY` (#1048) |
| `bili copilot [opts --] [args]` | Proxy + **Copilot CLI** (GitHub, closed Go binary) — cert-MITM via `HTTPS_PROXY` + `SSL_CERT_FILE`; model hosts (`api.githubcopilot.com` + per-plan subdomains) whitelisted (#1049) |
| `bili amp [opts --] [args]` | Proxy + **Amp CLI** (Sourcegraph, closed Go binary) — cert-MITM via `HTTPS_PROXY` + `SSL_CERT_FILE`; `ampcode.com` whitelisted (#1049) |
| `bili goose [opts --] [args]` | Proxy + **Goose** (Block, Rust/reqwest) — rustls release builds trust no CA file, so no proxy envs: built-in openai/anthropic legs redirected via `OPENAI_HOST`/`ANTHROPIC_HOST`, custom declarative providers via a regenerated `GOOSE_PATH_ROOT` overlay with `base_url` `/bili/` rewrites (real config untouched, user edits merged back); fixed third-party providers warned about (#1049) |
