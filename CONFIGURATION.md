...
| `BILI_LOG_MASK_HOSTS` | Set `0` to turn OFF host masking in proxy logs (#897): non-public target hosts (private relays, internal domains) are logged verbatim instead of `<private-host>`. Default is ON (#255 — logs get pasted into public issues); credential-header masking is independent and always on. Real target hosts are always available without touching this flag: `GET /__bili/stats` → `blindTunnels`, `GET /__bili/health` (both loopback-only), and the `acp_status` output. |
 ...
 | `BILI_STABLE_SYSTEM_ANCHOR` | Set `1` to enable stable-system anchoring (#1085): bili remembers each session's first-seen head system/instructions block and keeps re-sending those exact bytes even when the client's system prompt later changes — a detected change is appended instead as a trailing `[System context update] …` user message carrying the new full text (each update supersedes all previous ones). This preserves the upstream provider's KV prefix cache when clients embed volatile files (AGENTS.md & co.) in the system prompt; otherwise every change invalidates the whole cached prefix. The anchor and update log persist with the session and survive compression/compaction (they live in session metadata, not kernel state). Churn guard: more than 8 distinct changes replace the anchor outright with the newest text (one deliberate cache miss) and clear the log. Excluded from anchoring: title-gen micro-requests (OpenAI/Google), Responses compaction-trigger requests, and auto-mode classifier requests. Clients that implement their own variant of this mechanism (stable prompt + in-history updates) need no extra injection — such updates pass through as ordinary history. Default is OFF. `"stableSystemAnchor": true` in the config file does the same; the env var wins. |
