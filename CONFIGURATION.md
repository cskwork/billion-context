...
| `BILI_SESSION_GC` | Set `0` to disable automatic cleanup of stale session files (#1082). Default is ON whenever persistence is enabled. Short-lived sessions leave small state files that are never resumed; the sweep deletes files that are both old and small — resuming the conversation rebuilds the context from the client's own history, so losing the file costs at most one cold rebuild. Safety rails: files younger than the age threshold are never touched; unreadable/corrupt files are left in place (logged); a file belonging to a session still held in memory is skipped unless that session has been idle since its last disk write; emptied protocol subdirectories are removed; one summary log line per sweep that removed anything. |
