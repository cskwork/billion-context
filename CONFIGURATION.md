...
| `BILI_CLAUDE_BEDROCK` | `bili claude` Bedrock mode: `1` forces it (even when settings say `CLAUDE_CODE_USE_BEDROCK=0`), `0` disables it. Unset = follow claude's own `CLAUDE_CODE_USE_BEDROCK` (settings env block, then shell). See [Claude on AWS Bedrock](#claude-on-aws-bedrock). |
...
When claude runs on Bedrock (`CLAUDE_CODE_USE_BEDROCK` truthy in `~/.claude/settings.json` env or the shell, or `BILI_CLAUDE_BEDROCK=1`), `bili claude` points claude's `ANTHROPIC_BEDROCK_BASE_URL` at `/bili/https://bedrock-runtime.<AWS_REGION>.amazonaws.com` (or at your configured `ANTHROPIC_BEDROCK_BASE_URL` gateway) — via the spawn env and `--settings`, never by editing your settings file. The proxy accepts `POST /model/{id}/invoke-with-response-stream` and `/invoke`, runs the normal Anthropic pipeline on the body (model and stream come from the path), sends it back upstream in Bedrock shape, and converts the `application/vnd.amazon.eventstream` response in both directions (CRC-checked frames; Bedrock exception frames reach claude unchanged). The `Authorization` header is forwarded untouched: bearer-token auth (`AWS_BEARER_TOKEN_BEDROCK`) works; SigV4-signed requests cannot survive a body rewrite. The long-context beta Bedrock carries in the body (`anthropic_beta: ["context-1m-…"]`) sizes the window like the header does on the Anthropic wire. Model-discovery calls (`/inference-profiles`, `/foundation-models`) are routed to the regional control-plane host; every other Bedrock path is forwarded unchanged. Opt out with `BILI_CLAUDE_BEDROCK=0`.
