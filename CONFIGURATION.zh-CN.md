...
| `BILI_SESSION_GC` | 设 `0` 关闭过期会话文件的自动清理（#1082）。持久化开启时默认 ON。短命会话会留下永远不会再被恢复的小状态文件；清理只删**既老又小**的文件 —— 继续对话会用客户端自己的历史重建上下文，丢文件最多损失一次冷重建。安全边界：未到年龄阈值的文件绝不碰；不可读/损坏的文件原地保留（有日志）；内存中仍持有的会话对应的文件会被跳过，除非该会话自上次落盘后一直空闲；清空后的协议子目录一并删除；每次有删除的扫描输出一条汇总日志。 |
