...
| `BILI_LOG_MASK_HOSTS` | 设为 `0` 关闭代理日志的 host 脱敏（#897）：非公开目标主机（私有 relay、内网域名）原样记录，而不是 `<private-host>`。默认开启（#255 —— 日志常被整段贴进公开 issue）；凭据头脱敏与之独立、始终开启。真实目标域名不依赖此开关也可查：`GET /__bili/stats` → `blindTunnels`、`GET /__bili/health`（均仅 loopback），以及 `acp_status` 输出。 |
 ...
 | `BILI_STABLE_SYSTEM_ANCHOR` | 设为 `1` 开启稳定 system 锚定（#1085）：bili 按会话记住客户端首次发送的头部 system/instructions 块，即使客户端之后修改了 system prompt，也继续原样重发这些字节 —— 检测到的变更改为追加为末尾的 `[System context update] …` user 消息（内含新全文，每次更新取代之前所有更新）。当客户端把易变文件（如 AGENTS.md）嵌进 system prompt 时，这能保住上游 provider 的 KV 前缀缓存；否则每次变更都会使整个已缓存前缀失效。锚点与更新日志随会话持久化，且不受压缩/compaction 影响（存放在 session metadata 而非 kernel state 中）。防抖保护：累积超过 8 次不同变更后，锚点直接替换为最新文本（一次有意的缓存失效）并清空日志。不参与锚定的请求：标题生成微请求（OpenAI/Google）、Responses compaction-trigger 请求、auto-mode classifier 请求。若客户端自身已实现同类机制（稳定 prompt + 历史内更新），则无需额外注入 —— 这类更新作为普通历史透传。默认关闭。配置文件中设 `"stableSystemAnchor": true` 效果相同；环境变量优先。 |
